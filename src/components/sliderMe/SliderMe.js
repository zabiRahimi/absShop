...
const SliderMe = ({ className, slides, typeClass, title, symbol }) => {
    ...
    */
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            ...
            if (widthSliderMe > widthBodySlider) {
                ...
                const btnElements = Array.from(document.getElementsByClassName('divBtnMe'));
    ...
    useEffect(() => {
         ...
         */
        setValueTranslate(bodySlider.current.querySelector(`.${className}`).offsetWidth);
        ...
        const elements = Array.from(bodySlider.current.getElementsByClassName('divBtnMe'));
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            ...
            const slide = sliderMe.current.querySelector(`.container_${typeClass}_me`);
            ...
            const padingProduct = parseFloat(styleProduct.paddingRight) + parseFloat(styleProduct.paddingLeft);
            ...
            const widthSlides = ((widthSlide + marginRightSlide + marginLeftSlide) * Object.values(slides).length) + (widthTitle + marginRightTitle + marginLeftTitle + widthMore + marginRightMore + marginLeftMore + padingProduct);
    ...
    const handleRight = () => {
        ...
        if (valueTransform > 0) {
            ...
            const valueTranslateX = valueTransform > valueTranslate ? valueTransform - valueTranslate : 0;
    ...
    const handleLeft = () => {
        ...
        if (check < 0) {
            ...
            const lastTranslateX = ((valueTransform + valueTranslate) - (check + valueTranslateLast));
    ...
    const handleEndSelect = () => {
        ...
        if (isMouseDown) {
            ...
            } else if (keeperMove < 0 && keeperMove > (startX < keeperEndX ? -(controlLimit - showMore) : -controlLimit)) {

                transform(-keeperMove);

                if (keeperEndX > startX) { displayButton('btnRight', 'block'); }
                if (keeperEndX < startX) { displayButton('btnLeft', 'block'); }
    ...
    */
    ...
    const setTransition = (boolean) => {
        ...
        const select = Array.from(sliderMe.current.getElementsByClassName(`${className}`));
    ...
    return (
        <div className="bodySlider" ref={bodySlider}>
            ...
            <div className='divBtnMe divBtnMeRight notShowDivBtnMe' id={`btnRight${className}`}>
                <button onClick={handleRight} className='btnMe' ><i className="icofont-curved-right" ></i></button>
            </div>
            ...
            <div
                ...
                onTouchStart={isActiveEvent ? event => handleStartSelect(event, true) : undefined}

                onTouchMove={isActiveEvent ? (event) => {
                    if (isMouseDown) {
                ...
                onMouseDown={isActiveEvent ? (event) => { handleStartSelect(event, false) } : undefined}

                onMouseMove={isActiveEvent ? (event) => {
                    if (isMouseDown) {
            ...
            >
                ...
                <div className={`slideMe title_${typeClass}_me`} id={`title${className}`}>
                    <h2 className={`titleH2_${typeClass}_me`} >
                        {title}
                    </h2>
                    <div className={`divTitleImg_${typeClass}_me`}>

                        <img src={require(`../../assets/images/products/${symbol}.jpg`)} alt={symbol} className={`titleImg_${typeClass}_me`} />
                ...
                {Object.values(slides).map((slide, i) => (
                    ...
                    <article key={slide.name} id={`${className}${i}`} className={`slideMe ${className} container_${typeClass}_me ${i === 0 ? 'firstContainer_' + typeClass + '_me' : ''} `}>

                        <div className={`divImg_${typeClass}_me`}>

                            <img className={`img_${typeClass}_me`} src={require(`../../assets/images/products/${slide.img}`)} alt={slide.name} />
                ...
                <div className={`slideMe more_${typeClass}_me `} id={`more${className}`}>

                    <button className={`moreBtn_${typeClass}_me`}>

                        <span className={`moreBtnIcon_${typeClass}_me`}><i className='icofont-swoosh-down' /></span>
            ...
            <div className='divBtnMe divBtnMeLeft notShowDivBtnMe' id={`btnLeft${className}`}>

                <button onClick={handleLeft} className='btnMe'><i className="icofont-curved-left" ></i></button>
