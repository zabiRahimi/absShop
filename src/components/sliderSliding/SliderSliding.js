...
const SliderSliding = ({ title, row, more, bgColor, color, typeClass, id, paginate, slides, linkMore = '' }) => {
    ...
    useEffect(() => {
        ...
        const displayBtnNave = () => {
            ...
            const btnElements = Array.from(document.getElementsByClassName('divBtnNavSS'));
    ...
    useEffect(() => {
        ...
        const elements = Array.from(bodySliderSliding.current.getElementsByClassName('divBtnNavSS'));
        ...
        if (isMouseDown) {
            if (keeperMove > 0 && Math.abs(keeperMove) > 20) {
                ...
                if (numStepRight > 0) {
                    ...
                    event.currentTarget.style.transform = `translateX(${((valueTranslate * numStepRight) - valueTranslate)}px)`;
                ...
                if (numPaginate - 1 > numStepLeft) {
                    event.currentTarget.style.transform = `translateX(${valueTranslate * numStepLeft}px)`;
            ...
            setTransformValue(event.currentTarget.style.transform.match(/-?[\d\.]+/g)[0]);
        }
    ...
    const setElement = Array.from({ length: column }).map((_, a) => {
        ...
        return <div className='continerColumnSS' ref={containerColumnSS} key={a} id={`containerCol_${id}_${a}_SS`}>
            {
                ...
                Array.from({ length: row }).map((_, index) => {
                    ...
                    if (!slides[`slide${count}`]) { return }

                    return (<article className='containerSlide' key={index}>

                        <div className={`divImg_${typeClass}_SS`}>
                            <img src={require(`../../assets/images/products/${slides[`slide${count}`].img}.webp`)} alt='' className={`img_${typeClass}_SS`} />
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            ...
            const continerSliderSliding = document.getElementById(`continerSliderSliding_${id}_SS`);
            ...
            if (containerCol) {
                ...
                setNumPaginate(Math.ceil(Object.values(slides).length / (numCol * row)));
    ...
    const handleMoveBtn = (event, side) => {
        if (side == 'right' && numStepRight > 0) {
            ...
            containerSliderSliding.current.style.transform = `translateX(${((valueTranslate * numStepRight) - valueTranslate)}px)`;

            setNumStepRight(prve => prve - 1);
            numStepLeft != numStepRight && setNumStepLeft(prve => prve - 1);

        } else if (side == 'left' && numStepLeft < numPaginate - 1) {
            containerSliderSliding.current.style.transform = `translateX(${valueTranslate * numStepLeft}px)`;
            setNumStepLeft((prve) => prve + 1);
            setNumStepRight((prve) => prve + 1);
        ...
        containerSliderSliding.current.style.transition = 'transform 0.9s ease-in-out';
        setTransformValue(containerSliderSliding.current.style.transform.match(/-?[\d\.]+/g)[0]);
    ...
    return (
        <div className='bodySliderSliding' ref={bodySliderSliding} style={{ backgroundColor: bgColor }} id={`body_${id}_SS`} >
            ...
            <div className='notShowDivBtnSS divBtnNavSS divBtnNavRight' ref={navBtnRight}>
                <button className='btnNavSS btnNavRight' onClick={(event) => handleMoveBtn(event, 'right')}>
                    <i className='icofont-curved-right' />
                </button>
            </div>

            <div className='continerSliderSliding'
                ...
                onMouseUp={isActiveEvent ? (event) => handleEndSelect(event) : undefined}

                onMouseLeave={isActiveEvent ? (event) => handleEndSelect(event) : undefined}
            ...
            <div className='notShowDivBtnSS divBtnNavSS divBtnNavLeft' ref={navBtnLeft}>
                <button className='btnNavSS btnNavLeft' onClick={(event) => handleMoveBtn(event, 'left')}>
